...
def products(request):
    ...
    if filter_form.is_valid():
        ...
        if name:
            products = products.filter(name__icontains = name) #icontains is case insensitive, either the name is contains the search term or not

    # filtering by min price
    if filter_form.is_valid():
        min_price = filter_form.cleaned_data.get("min_price")
        if min_price is not None:
            products = products.filter(price__gte = min_price) #gte means greater than or equal to

    # filtering by max price
    if filter_form.is_valid():
        max_price = filter_form.cleaned_data.get("max_price")
        if max_price:
            products = products.filter(price__lte = max_price) #lte means less than or equal to

    # filtering by category
    if filter_form.is_valid():
        categories = filter_form.cleaned_data.get("categories")
        if categories:
            products = products.filter(categories__in = categories) #__in is used to filter by multiple values
...
def add_to_cart(request, pk): #pk means primary key
    try:
        ...
        quantity = int(request.POST.get("quantity")) #default quantity is 1 if not provided
        if quantity < 0:
            messages.error(request, "Quantity can not be Zero.")
            return redirect("store:product_detail_page", pk=pk)
        
        # cart.products.add(product)
        CartProduct.objects.create(cart=cart, product=product, quantity=quantity) #creating a through model instance
